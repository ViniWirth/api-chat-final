...
app.use(
  ...
  router.get("/salas", async (req, res, next) => {
    ...
    const test = await token.checkToken(
      req.headers.token,
      req.headers.iduser,
      req.headers.nick
    );
...
app.use(
  ...
  router.post("/sala/entrar", async (req, res) => {
    ...
    if (
      ...
      res.status(200).send({ msg: "Entrou na sala", timestamp: Date.now() });
    } else {
      res.status(401).send({ msg: "Usuário não autorizado" });
    }
  })
...
app.use(
  "/sala/sair",
  router.get("/sala/sair", async (req, res) => {
    res.json({ status: 200, msg: "Saiu da sala" });
  })
);
module.exports = app;
